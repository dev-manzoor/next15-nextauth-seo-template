...
export class Validator {
  ...
  addRule<T = unknown>(field: string, rule: ValidationRule<T>): Validator {
    if (!this.rules[field]) {
      this.rules[field] = [];
    }
    this.rules[field].push(rule as ValidationRule<unknown>);
    return this;
  }
...
export const validationRules = {
  ...
  numeric: (message = "Must be a number"): ValidationRule => ({
    validate: (value) =>
      !Number.isNaN(Number(value)) && !Number.isNaN(parseFloat(String(value))),
    message,
  }),
